...
namespace exams
{
    public partial class frmComputeResults
    {
        public frmComputeResults()
        {
            InitializeComponent();
            _chkMarksGrade.Name = "chkMarksGrade";
            _btnIndexing.Name = "btnIndexing";
        ...
        private List<Tuple<string, int>> getHighestMark(Array subjectList, int rowkcpe)
        {
            var highestMarks = new List<Tuple<string, int>>();
            var subjectMarkss = new List<Tuple<string, int>>();
            foreach (string s in subjectList)
                subjectMarkss.Add(new Tuple<string, int>(s, Conversions.ToInteger(dgvMeanMarks.Rows[rowkcpe].Cells[s].Value)));
        ...
        private string getSelectStatement(string stude_admno)
        {
            string statements = string.Empty;
            foreach (string S in publicSubsNFunctions.subjectColumns)
            {
                statements += "round(";
                for (int j = 0, loopTo = publicSubsNFunctions.examListMain.Count - 1; j <= loopTo; j++)
                    statements += "sum((select ((" + S + "/" + publicSubsNFunctions.examListMain[j].Item5 + ")*" + publicSubsNFunctions.examListMain[j].Item2 + ") from exam_results where admno = " + stude_admno + " and (examination = '" + publicSubsNFunctions.examListMain[j].Item1 + "' and term = '" + publicSubsNFunctions.examListMain[j].Item3 + "' and year = " + publicSubsNFunctions.examListMain[0].Item4 + " ))) +";
        ...
        private void chkMarksGrade_CheckedChanged(object sender, EventArgs e)
        {
            if (chkMarksGrade.CheckState == CheckState.Checked)
            {
                showMarksAndGrade();
            }
            else
        ...
        private void setIndexing()
        {
            ...
            if (Conversions.ToBoolean(publicSubsNFunctions.confirm("Are You Sure ?")))
            {
                ...
                using (new DevExpress.Utils.WaitDialogForm("Saving Data Please Wait"))
                    for (int i = 0, loopTo = dgvMeanMarks.Rows.Count - 1; i <= loopTo; i++)
                    {
                        kcpe = Conversions.ToString(Operators.AddObject(code, dgvMeanMarks.Rows[i].Cells["OP"].Value));
                        if (!publicSubsNFunctions.qwrite(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject("UPDATE students SET kcpeno = '" + kcpe + "' WHERE admin_no =  '", dgvMeanMarks.Rows[i].Cells["admno"].Value), "'"))))
                        {
                            publicSubsNFunctions.rollback();
                            publicSubsNFunctions.failure("Could Not Successfully Perform kcpe Numbering!");
                            return;
                        }
                    }
        ...
        private void frmComputeResults_Load_1(object sender, EventArgs e)
        {
                    ...
                    for (int i = 0, loopTo = students.Count - 1; i <= loopTo; i++)
                    {
                        ...
                        if (publicSubsNFunctions.qread(ref sqlStatment))
                        {
                            if (publicSubsNFunctions.dbreader.RecordsAffected > 0)
                            {
                                ...
                                {
                                    allResults.Add(new Tuple<string, double>(students[i].Item5, tmm / (double)subjectEntries));
                                }
                                else if (publicSubsNFunctions.sortGradesBy == "Mean Points")
                                {
        ...
        {
            for (int k = 0, loopTo = publicSubsNFunctions.examListMain.Count - 1; k <= loopTo; k++)
                publicSubsNFunctions.exam_name += publicSubsNFunctions.examListMain[k].Item1 + "/";
